...
inequality_resolution([Quantity, _, _], [ObjectName, ObjectMagnitude, ObjectDerivative], '+'):-
    ObjectDerivative == '+',
    not(inconsistent_derivative([ObjectName, ObjectMagnitude, '+'])),
    write("By inequality reasoning the ambiguous state is being resolved..."),
    nl,
    write("We assume the following quantity is less influential: "),
    write(Quantity),
    nl.
...
resolution(State, ResultState):-
    resolution(State, State, ResultState).
...
next_state(State, NextState):-
    transition_state(State, TransitionState, Trace),
    resolution(TransitionState, TransitionState, NextState), nl,
    explain(Trace),
    write("PreviousState = "), write(State), nl.
